...
def handle_message_entity(data_folder, args, formatter):
    ...
    if hasattr(args, 'message_fields'):
        if args.message_fields:
...
def set_args():
    ...
    root_parser.add_argument("pstfile", help="PST file to be parsed", type=lambda x: is_valid_file(root_parser, x), metavar="PST_FILE")
