...
class PSTFolder(object):
    ...
    def get_messages_iter(self, max_messages=1000):
        ...
        for i in range(0, max_messages_returned):
            try:
                msg = PSTMessage(i, self.folder.get_sub_message(i), self.folder)
                yield msg
            except:
                continue
...
class PSTRecordEntry:
    ...
    value_type_funcs = {
        ...
        0x001f: lambda x:(pypff.record_entry.get_data_as_string(x)).decode("utf-8"), #LIBPFF_VALUE_TYPE_STRING_UNICODE
